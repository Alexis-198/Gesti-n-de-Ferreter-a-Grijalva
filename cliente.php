...
$objetoConexion = new clasecliente();
...
if (isset($_GET['editar'])) {
    $res = $objetoConexion->seleccionarcliente((int)$_GET['editar']);
...
$recibircliente = $objetoConexion->obtenercliente();
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<table>
    ...
    <?php
    $i = 0;
    while ($row = $recibircliente->fetch_assoc()):
        $i++;
    ?>
        <tr>
            <td><?= $i ?></td>
            ...
            <td>
                <a href="?editar=<?= (int)$row['id_cliente'] ?>">✏️ Editar</a>
                <a href="?eliminar=<?= (int)$row['id_cliente'] ?>"
                   onclick="return confirm('¿Eliminar este cliente?')">🗑️ Eliminar</a>
            </td>
        </tr>
    <?php endwhile; ?>
    <?php if ($i === 0): ?>
